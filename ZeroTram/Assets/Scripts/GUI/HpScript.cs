...
public class HpScript : MonoBehaviour
{
    ...
    private const float _baraban_y = 23.8f;
    // Use this for initialization
    void Start()
    {
        _num_posY = Num_Right.transform.position.y;
    }
        ...
        else
        {
            if (_hp > _hp_old)
            {
                if (xx > 0)
                {
                    ...
                    {
                        Num_Middle.transform.position = new Vector3(Num_Middle.transform.position.x, _num_posY - (_baraban_y * xx), Num_Middle.transform.position.z);
                        _bar_middle = false;
                    }
                }
                else
            ...
            if (_hp < _hp_old)
            {
                ...
                {
                    if (Num_Middle.transform.position.y > (_num_posY - (_baraban_y * 9)))
                        Num_Middle.transform.Translate(0, -0.45f * _baraban_speed, 0);
                    else
                    {
                        Num_Middle.transform.position = new Vector3(Num_Middle.transform.position.x, _num_posY - (_baraban_y * 9), Num_Middle.transform.position.z);
                        _bar_middle = false;
                    }
                }
                if (x < 9)
